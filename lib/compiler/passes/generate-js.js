...
function generateJS(ast, options) {
  ...
  function generateRuleHeader(ruleNameCode, ruleDescCode, ruleIndexCode) {
    var parts = [];
    ...
    if (options.trace) {
      parts.push([
        ...
        '  type:        "rule.enter",',
        '  rule:        ' + ruleNameCode + ',',
        (ruleDescCode ? '  description: ' + ruleDescCode + ',' : ''),
  ...
  function generateRuleFooter(ruleNameCode, ruleDescCode, resultCode) {
    var parts = [];
  ...
  function generateInterpreter() {
    ...
    parts.push(indent2(generateRuleHeader('peg$ruleNames[index]', 'peg$descNames[index]', 'index')));
    ...
    parts.push(indent2(generateRuleFooter('peg$ruleNames[index]', 'peg$descNames[index]', 'stack[0]')));
    parts.push('}');
  ...
  function generateRuleFunction(rule) {
    ...
    var ruleNameEsc = '"' + js.stringEscape(rule.name) + '"';
    var ruleDescEsc = descNames[rule.name] != null ?
        '"' + js.stringEscape(descNames[rule.name]) + '"' : null;
  ...
  if (options.trace) {
    descNamesArr = ast.rules.map(function (r) {
      descNames[r.name] = r.expression.name;
  ...
  if (options.trace) {
    if (options.optimize === "size") {
      ruleNames = '['
                ...
                + ']';
      descNamesArrStr = '['
                + descNamesArr.map(function (d) {
                    return d != null ? '"' + js.stringEscape(d) + '"' : 'null';
                  }).join(', ')
                + ']';

      parts.push([
        '        peg$ruleNames = ' + ruleNames + ',',
        '        peg$descNames = ' + descNamesArrStr + ',',
        ''
      ].join('\n'));
    }
